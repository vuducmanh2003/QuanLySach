...
<div id="page-content" class="single-page">
   ...
   LEFT JOIN nhaxuatban n on n.ID = s.Manhasx WHERE  s.ID =' . $id;
	...
	if (isset($_POST['submit'])) {
		...
		echo "<script>window.location.replace('http://localhost:8888/qls/cart.php'); </script>";
	}
				...
				<div class="product">
					...
					<div class="col-md-6">
						<div class="caption">
							...
							?>
								<div class="price"><?php echo $row["giakhuyenmai"] ?> VNĐ<span><?php echo $row["Gia"] ?> VNĐ</span></div>
							<?php
							}
							?>
							<?php
							...
							?>
								...
								<div class="price"><?php echo $row["Gia"] ?> VNĐ<span></span></div>
							<?php
							}
							?>
							...
							<div class="well">
								<form name="form3" id="ff3" method="POST" action="">
									<input type="submit" name="submit" id="add-to-cart" class="btn btn-addcart" value="Thêm vào giỏ hàng" />
									<a href="#" class="btn btn-buy" data-toggle="modal" data-target="#myModal">Mua ngay</a>
											...
											<form name="form6" id="ff6" method="POST" action="<?php include "luumuangay.php" ?>">
												...
												<div class="form-group">
													<label> Hình thức thanh toán :<select class="selectpicker" name="hinhthuctt">
															<option value="Trực tuyến">Chuyển khoản ngân hàng</option>
												...
												<button type="submit" name="muangay" class="btn btn-buy">Đặt hàng</button>
											</form>
										</div>
